 ...
 * that sets the transfer-gateway.url property to point to the MockServer container.
 * 
 * Usage:
 * ```
 * @ContextConfiguration(initializers = [MockServerProperties.Initializer::class])
 * ```
 */
open class MockServerConfig {
    companion object {
        ...
        /**
         * Sets up an expectation for the MockServer to respond to a POST request to /api/accounts
         * with a 201 CREATED response.
         *
         * @param responseBody The response body to return (true for success, false for already exists)
         * @param statusCode The HTTP status code to return (201 for created, 409 for conflict)
         */
        fun setupAddMonitoredAccountExpectation(responseBody: Boolean, statusCode: Int = 201) {
            try {
                createMockServerClient()
                    .`when`(
                        HttpRequest.request()
                            .withMethod("POST")
                            .withPath("/api/accounts-to-watch")
                    )
                    .respond(
     ...
     * This initializer sets the transfer-gateway.url property to point to the MockServer container.
     */
    class Initializer : ApplicationContextInitializer<ConfigurableApplicationContext> {
        override fun initialize(applicationContext: ConfigurableApplicationContext) {
            ...
            val properties = mapOf("transfer-gateway.url" to mockServerUrl)
            ...
            System.setProperty("transfer-gateway.url", mockServerUrl)
        }
    }
}
